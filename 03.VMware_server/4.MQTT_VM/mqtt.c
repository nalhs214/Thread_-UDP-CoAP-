 ...
 * 브로커 설정
 *==============================================*/
#define BROKER_HOST  "192.168.1.94"  /* RPi 브로커 주소 */
...
#define TOPIC_SUB    "nucode/rx"     /* RPi → VM 수신 토픽 */
...
{
    /* RPi로부터 수신한 JSON 데이터 출력 */
    printf("[수신] 토픽: %s | 데이터: %s\n",
...
struct mosquitto *mqtt_init(void)
{
    ...
    /* 2. 클라이언트 인스턴스 생성 */
    mosq = mosquitto_new("vm_client", true, NULL);
    if(mosq == NULL) {
